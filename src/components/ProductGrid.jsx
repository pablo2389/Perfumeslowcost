...
const ProductGrid = ({ category, cart, setCart, fullView }) => {
  ...
  const filteredPerfumes =
    category === "todos"
      ...
      : category === "marcas"
      ? perfumes.filter((p) => p.marca) // Puedes filtrar o agrupar marcas si quieres
      : perfumes;
  ...
  const scroll = (direction) => {
    ...
    const scrollAmount = 250; // Ancho aproximado tarjeta
                ...
                <Box sx={{ mt: 1, display: "flex", alignItems: "center" }}>
                  <TextField
                    ...
                    onChange={(e) =>
                      setQuantities({
                        ...quantities,
                        [perfume.id]: parseInt(e.target.value) || 1,
                      })
                    }
                    sx={{ width: "80px", mr: 1 }}
                  />
  ...
  return (
    <Grid container spacing={2}>
      {filteredPerfumes.map((perfume) => (
        <Grid key={perfume.id} item xs={12} sm={6} md={4}>
          <Card sx={{ height: "100%" }}>
            <CardMedia
              component="img"
              height="220"
